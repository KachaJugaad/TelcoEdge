#!/usr/bin/env python3
"""Update the CanEdge test dashboard with latest benchmark results.

Reads reports/latest_benchmark.json and generates docs/canedge-testview.html.
...
def generate_html(benchmark: dict, phase: dict) -> str:
...
def main():
    ...
    phase = load_phase()
    html = generate_html(benchmark, phase)

    DASHBOARD_FILE.parent.mkdir(parents=True, exist_ok=True)
    DASHBOARD_FILE.write_text(html)

    print("Dashboard update summary:")
    print(f"  Source:      reports/latest_benchmark.json")
    print(f"  Output:      docs/canedge-testview.html")
    print(f"  Phase:       {phase.get('phase', '?')} ({phase.get('pct_complete', 0)}%)")
    print(f"  Scenario:    {benchmark.get('scenario', 'N/A')}")
    print(f"  Terrain:     {benchmark.get('terrain_type', 'N/A')}")
    print(f"  Weather:     rain {benchmark.get('rain_mm_per_hr', 0)} mm/hr")
    print(f"  Runs:        {benchmark.get('n_runs', 0)}")
    print(f"  BER fixed:   {benchmark.get('mean_ber_fixed_mcs', 0):.4e}")
    print(f"  BER adaptive:{benchmark.get('mean_ber_adaptive_mcs', 0):.4e}")
    print(f"  Improvement: {benchmark.get('ber_improvement_pct', 0):.1f}%")
    print(f"  Status:      {'PASS' if benchmark.get('adaptive_is_better') else 'FAIL'}")
